...
### 1. 관리자 회원 삭제 기능 정상화 (Critical)
- **증상**: 회원 삭제 시 500 에러 발생.
- **원인**: `user_inventory_item` 등 연관 테이블에 `ondelete="CASCADE"` 설정이 누락되어 DB 제약 조건에 의해 삭제가 차단됨.
- **조치 예정**: 인벤토리 및 활동 로그 모델에 CASCADE 설정을 추가하여 회원 삭제 시 연관 데이터가 자동으로 정리되도록 수정 필요.
