...
const rouletteSchema = z
  .object({
    ...
    segments: z.array(segmentSchema).length(6, "세그먼트는 6개 모두 입력하세요"),
  })
...
const RouletteConfigPage: React.FC = () => {
  ...
  return (
    <section className="space-y-6">
      ...
      {isLoading && <div className="rounded-lg border border-emerald-700/40 bg-slate-900 p-4 text-slate-200">불러오는 중...</div>}
      {isError && (
        <div className="rounded-lg border border-red-500/40 bg-red-950 p-4 text-red-100">불러오기 실패: {(error as Error).message}</div>
      )}

      {!isLoading && data && data.length === 0 && (
        <div className="rounded-lg border border-emerald-700/40 bg-slate-900 p-4 text-slate-200">등록된 설정이 없습니다.</div>
      )}
        ...
        <form className="space-y-4" onSubmit={onSubmit}>
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            ...
            <div className="flex items-center space-x-3 pt-6">
              ...
              <span className="text-sm text-slate-200">활성</span>
            </div>
          </div>
            ...
            <div className="space-y-2">
              {fields.map((field, idx) => (
                <div key={field.id} className="grid grid-cols-1 gap-2 rounded-md border border-emerald-800/50 bg-slate-900 p-3 md:grid-cols-6">
                  <div className="space-y-1">
                    <label className="text-xs text-slate-300">Index (자동)</label>
                    <input
                      type="number"
                      readOnly
                      value={idx}
