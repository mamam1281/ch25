...
- 미션 탭에서 보상 수령(우측 트로피 아이콘 버튼) 시 보상 지급
[유저 화면 노출]
- **홈 화면**: 홈 진입 후 미션 조회 시, 미수령 상태면 "오늘의 선물 도착" 토스트 알림 (페이지 1회)
- **네비게이션**: 하단 '이벤트' 탭에 빨간색 핑(Ping) 알림 (미수령 미션 또는 시즌패스 보상 존재 시)
- **대시보드**: 이벤트 대시보드의 미션 카드에 빨간색 알림 아이콘(느낌표 역할) 노출
- **미션 탭**: [Daily] 카테고리에서 최상단 고정 + 골드 강조 배경 + Gift 아이콘
[보상 설계]
- **보상**: 10 다이아
...
[시나리오]
1) 접속 및 수령
- 로그인 -> 홈 화면 토스트 확인 -> 하단 '이벤트' 핑 확인 -> 이벤트 탭(대시보드)에서 미션 알림 아이콘 확인 -> 미션 탭 진입 -> '매일 선물' 최상단/골드 강조 확인 -> 우측 트로피 아이콘으로 수령 -> 다이아 10 증가
