...
- 스크립트 실행: bash 기반 scripts/backup.sh는 로컬에 bash 미존재로 실행 불가. python scripts/seed_test_data.py --help 실행 시 DB 호스트 'db' 미해결로 OperationalError(2003) 발생(로컬 MySQL 미기동 상태).
