...
- **핵심 변경**: Ticket Zero 발생 시 **`TRIAL_TOKEN`**(체험 토큰)을 지급합니다.
    - 기존: `ROULETTE_COIN` 등 실전 티켓 지급 (삭제/중단)
    - 신규: `TRIAL_TOKEN` 3장 지급
- **체험 루프 (Resurrection Loop)**:
    1. **체험**: `TRIAL_TOKEN`으로 **체험 룰렛(Practice Mode)** 플레이.
    2. **채굴**: 체험 룰렛 승리 시 **`DIAMOND`**(다이아몬드) 획득 (확률).
    3. **환전**: 상점에서 다이아로 **일반 게임 연료**(`ROULETTE_COIN` / `DICE_TOKEN`) 교환권 구매.
    4. **진입**: 교환권 사용 후 실전 게임 진입 → 금고(`Vault`) 적립.
- **운영 효과**: 
...
graph TD
    ...
    subgraph "Resurrection Loop (Trial)"
        TicketZero --"Grant"--> TrialToken[🪙 Trial Token]
        TrialToken --"Play Trial"--> TrialGame[Practice Roulette]
        TrialGame --"Win"--> Diamond
    end
```
